...
class CPlayer():
	#创建角色
	def __init__(self, name, camp, occupation, passport_id):
...
def create_player(name, camp, occupation, passport_id):
	#创建角色
	retVal = {}
	if check_name(name):
		if check_camp(camp):
			if check_occupation(occupation):
				if check_passport_id(passport_id):
					p = CPlayer(name, camp, occupation, passport_id)
