            ...
            if (len(self._buffer) > 5):
                ...
                if flag == 0:
                    if (len(self._buffer[5:])>= len(self._buffer[5:5+targetLen])):
                        ...
                        print 'count: ', count
                        if(len(self._buffer[9:])>=len(self._buffer[9:9+count*4])):
                            player_id_list=[]
                            for i in xrange(count):
                                print 'i: ', i
                                (player_id,) =struct.unpack("!I",self._buffer[9+4*i:9+4*(i+1)]) 
                                player_id_list.append(player_id)
                                print 'player_id %s %s' % (i,player_id)
                                print 'len 1: ', len(self._buffer[9+count*4:])
                                print 'len 2: ', len(self._buffer[9+count*4:9+count*4+1+4+4]) 
                                print repr(self._buffer[9+count*4:])
                                print repr(self._buffer[9+count*4:9+count*4+1+4+4]) 
                            if len(self._buffer[9+count*4:])>=len(self._buffer[9+count*4:9+count*4+1+4+4]):
                                (seprator, msg_channel, cmd, amf3_data_len) = struct.unpack("!sHHI", self._buffer[9+count*4:9+count*4+1+4+4])
                                ...
                                if len(self._buffer[9+count*4+1+4+4:])>=len(self._buffer[9+count*4+1+4+4:9+count*4+1+4+4+amf3_data_len]):
                                    amf3_data_fmt = '%ss' % (amf3_data_len)
                                    (amf3_data,) = struct.unpack(amf3_data_fmt, self._buffer[9+count*4+1+4+4:9+count*4+1+4+4+amf3_data_len])
